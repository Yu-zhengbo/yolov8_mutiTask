...
class YOLO(object):
    ...
    def __init__(self, **kwargs):
        ...
        #   画框设置不同的颜色
        #---------------------------------------------------#
        hsv_tuples = [(x / max(self.num_classes), 1., 1.) for x in range(max(self.num_classes))]
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        with torch.no_grad():
            ...
            outputs = [self.bbox_util.decode_box(i) for i in outputs]
            #---------------------------------------------------------#
            #   将预测框进行堆叠，然后进行非极大抑制
            #---------------------------------------------------------#
            results = [self.bbox_util.non_max_suppression(outputs[i], self.num_classes[i], self.input_shape,
                        image_shape, self.letterbox_image, conf_thres = self.confidence, nms_thres = self.nms_iou) for i in range(len(outputs))]
            ...
            # top_labels   = [np.array(result[0][:, 5], dtype = 'int32') for result in results]
            # top_confs    = [result[0][:, 4] for result in results]
            # top_boxess   = [result[0][:, :4] for result in results]
        #---------------------------------------------------------#
        #   设置字体与边框厚度
        #---------------------------------------------------------#
        ...
        for q in range(len(outputs)):
            ...
            for i, c in list(enumerate(top_labels[q])):
                predicted_class = self.class_names[q][int(c)]
                box = top_boxess[q][i]
                score = top_confs[q][i]
