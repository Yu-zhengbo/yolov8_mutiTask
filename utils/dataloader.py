...
class YoloDataset(Dataset):
    def __init__(self, annotation_lines, input_shape, num_classes, epoch_length, \
                        mosaic, mixup, mosaic_prob, mixup_prob, train, special_aug_ratio = 0.7,task=['roadsign_voc','sagittaria_v1_voc']):
        ...
        self.bbox_attrs         = [5+i for i in num_classes]


        self.annotation_lines_copy = [[] for i in task]
        for i in self.annotation_lines:
            for j in range(len(task)):
                if task[j] in i:
                    self.annotation_lines_copy[j].append(i)
                    break
    ...
    def __getitem__(self, index):
        ...
        tag = self.annotation_lines[index].split()[0]
        for i in range(len(self.task)):
            if self.task[i] in tag:
                tag = i
                break
        #---------------------------------------------------#
        #   训练时进行数据的随机增强
        ...
        if self.mosaic and self.rand() < self.mosaic_prob and self.epoch_now < self.epoch_length * self.special_aug_ratio:
            ...
            lines = sample(self.annotation_lines_copy[tag], 3)
            ...
            if self.mixup and self.rand() < self.mixup_prob:
                ...
                lines = sample(self.annotation_lines_copy[tag],1)
...
def yolo_dataset_collate(batch):
    ...
    # if last_length == 0:
    for i in range(task):
        if len(images[i]) >= length:
            index_temp = np.random.choice(range(len(images[i])), size=length, replace=False)
        elif 0<len(images[i]) < length:
            index_temp = np.random.choice(range(len(images[i])), size=length, replace=True)
        else:
            index_temp = []
        if len(index_temp)==0:
            ...
            # bboxes[i] = np.array([])
            continue
        images[i] = np.array(images[i])[index_temp]
        bboxes[i] = [copy.deepcopy(v) for v in np.array(bboxes[i],dtype=object)[index_temp]]
    ...
    for i in bboxes:
        if len(i) > 0:
            bboxes_.append(torch.from_numpy(np.concatenate(i, 0).astype(float)).type(torch.FloatTensor))
        else:
            bboxes_.append(torch.from_numpy(np.array([])))
    # bboxes  = [torch.from_numpy(np.concatenate(i, 0).astype(float)).type(torch.FloatTensor) for i in bboxes]
