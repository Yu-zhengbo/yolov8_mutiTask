...
class Loss:
    def __init__(self, model): 
        ...
        self.assigner = TaskAlignedAssigner(topk=10,
                                            ...
                                            alpha=0.5,
                                            beta=6.0,
                                            roll_out_thr=self.roll_out_thr)
    ...
    def __call__(self, preds, batch,tag_temp):
        # 获得使用的device

        self.assigner = TaskAlignedAssigner(topk=10,
                                            num_classes=self.nc[tag_temp],
                                            alpha=0.5,
                                            beta=6.0,
                                            roll_out_thr=self.roll_out_thr)
        ...
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no[tag_temp], -1) for xi in feats], 2).split((self.reg_max * 4, self.nc[tag_temp]), 1)
